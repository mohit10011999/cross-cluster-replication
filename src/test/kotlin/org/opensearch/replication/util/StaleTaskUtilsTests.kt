...
class StaleTaskUtilsTests : OpenSearchTestCase() {
    ...
    // validateNoTasksRemaining

    fun testValidateNoTasksRemaining_passesWhenNoPersistentTasksMetadata() {
        ...
        // null PersistentTasksCustomMetadata → should return without throwing
        StaleTaskUtils.validateNoTasksRemaining(clusterService, followerIndex)
    }

    fun testValidateNoTasksRemaining_passesWhenNoMatchingTasks() {
        ...
        tasks.addTask<PersistentTaskParams>(
            ...
            PersistentTasksCustomMetadata.Assignment("node1", "test")
        )
        setClusterStateWithTasks(tasks.build())
        StaleTaskUtils.validateNoTasksRemaining(clusterService, followerIndex)
    }

    fun testValidateNoTasksRemaining_throwsWhenIndexTaskExists() {
        val tasks = PersistentTasksCustomMetadata.builder()
        tasks.addTask<PersistentTaskParams>(
        ...
        assertThatThrownBy {
            ...
            .hasMessageContaining("Replication tasks are still active for index $followerIndex")
    }

    fun testValidateNoTasksRemaining_throwsWhenShardTaskExists() {
        val tasks = PersistentTasksCustomMetadata.builder()
    ...
    fun testValidateNoTasksRemaining_throwsWhenUnassignedTaskExists() {
        val tasks = PersistentTasksCustomMetadata.builder()
        tasks.addTask<PersistentTaskParams>(
            "replication:index:$followerIndex",
            IndexReplicationExecutor.TASK_NAME,
